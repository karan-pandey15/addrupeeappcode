...
const DrawerList = [
  { icon: 'account', label: 'Check Status', navigateTo: '' }, 
  { icon: 'add', label: 'Help & Support', navigateTo: 'Help' },
  { icon: 'condition', label: 'Loan Terms', navigateTo: 'TermsAndConditions' },
  { icon: 'account-group', label: 'Refer Friend', navigateTo: 'ReferShare' },
  { icon: 'home-outline', label: 'Knowledge centre', navigateTo: 'knowledge' },
  { icon: 'home-outline', label: 'Improve Cibil', navigateTo: 'EmpForm' },
  { icon: 'home-outline', label: 'setting', navigateTo: '' },
];
...
function DrawerContent(props) {
  ...
  useEffect(() => {
    fetch("https://api.addrupee.com/api/get_customer_data")
  ...
  useEffect(() => {
    async function getEmail() {
      try {
        const storedData = await AsyncStorage.getItem('customerEmailID');
  ...
  const handleLogout = () => {
    fetch("https://api.addrupee.com/api/cust_logout")
      .then(response => response.json())
      .then(data => {
        if (data.Status === "Success") {
          AsyncStorage.clear();
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bottomDrawerSection}>
        <DrawerItem 
          // onPress={() => props.navigation.navigate('EmpRegister')}
